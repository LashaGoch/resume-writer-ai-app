...
def process_resume():
    ...
    tasks = [
        ...
        Task(
            ...
            expected_output="List of languages that starts with Languages:"
        )
    ...
    formatting_crew = Crew(agents=[formatter], tasks=[formatter_task], verbose=True)
    formatted_result = formatting_crew.kickoff()
    ...
    return f"""
        <div style='font-family: Calibri, sans-serif; padding: 20px;'>
            ...
            <pre style='white-space: pre-wrap; font-size: 14px; color: #333;'>{str(formatted_result).replace('<', '&lt;').replace('>', '&gt;')}</pre>
        </div>
    """
